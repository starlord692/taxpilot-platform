"""Reusable purchase invoice schema validators."""
...
def validate_purchase_dates(
    *,
    ...
    """Validate purchase due date is not before invoice date."""
...
def validate_total_at_least_subtotal(
    *,
    subtotal: Decimal | None,
    total_amount: Decimal | None,
) -> None:
    """Validate total amount is not less than subtotal when both are supplied."""
    if subtotal is None or total_amount is None:
        return
    if total_amount < subtotal:
        raise ValueError("total_amount must be greater than or equal to subtotal")
